...
-- Author: Joe Johnston <joe [at] airstack.io>
-- Source: https://github.com/kylemanna/lua-popen3
...
--
-- Wrapper around posix.execp with stdin, stdout, and stderr pipes.
--
-- Usage:
--   local pid, stdin_fd, stdout_fd, stderr_fd, exit_code = as.popen("ls", "-la", "/package/airstack")
--   print(posix.read(stdout, 10000))
--
-- @param cmd     Command to exec
-- @param args    One or more args to pass to exec
-- @return multi  pid, stdin_fd, stdout_fd, stderr_fd, exit_code;
--                pid and exit_code are numbers; the rest are posix file descriptors
-- @throws        When command fails
-- @see http://luaposix.github.io/luaposix/docs/modules/posix.html#execp
function M.popen3(path, ...)
...
--
function M.pipe_multi(input, max_procs, cmd, ...)
  ...
  while remaining > 0 do
    ...
    while inflight < max_procs and idx < #input do
      idx = idx + 1
      local lpid, lstdin_fd, lstdout_fd, lstderr_fd = M.popen3(cmd, ...)
      proc[idx] = {}
      proc[idx]['pid'] = lpid
...
--
function M.pipe_single(input, cmd, ...)
  local status, stdout, stderr = M.pipe_multi({input}, 1, cmd, ...)
